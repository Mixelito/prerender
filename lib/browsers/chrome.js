...
chrome.setUpEvents = function(tab) {
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			console.log(tab.prerender.userAgent || this.options.userAgent || this.originalUserAgent + ' Prerender (+https://github.com/prerender/prerender)');
			...
			Network.responseReceived((params) => {
				...
				if(tab.prerender.requests[params.requestId]) {
					...
					if (tab.prerender.logRequests || this.options.logRequests) util.log('-', params.requestId, tab.prerender.numRequestsInFlight, tab.prerender.requests[params.requestId]);
